...
import type {SavedPostsScreenProps} from '../navigation/types';
...
export default function SavedPostsScreen({navigation}: SavedPostsScreenProps) {
  ...
  const loadSavedPosts = useCallback(async () => {
    ...
    try {
      const snapshot = await firestore()
        .collection('posts')
        .where('savedBy', 'array-contains', state.uid)
  ...
  useEffect(() => {
    loadSavedPosts().finally(() => setLoading(false));
  }, [loadSavedPosts]);
  ...
  return (
    <SafeAreaView style={s.container}>
      <Header title="저장한 글" showBack onBack={() => navigation.goBack()} />
      {posts.length === 0 ? (
        <EmptyState icon="bookmark-outline" title="저장한 글이 없습니다" subtitle="마음에 드는 글을 저장해보세요!" />
      ) : (
        <FlatList
          data={posts}
