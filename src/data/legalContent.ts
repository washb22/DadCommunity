export interface LegalSection {
  title: string;
  content: string;
}

export interface LegalDocument {
  title: string;
  effectiveDate: string;
  lastModified: string;
  sections: LegalSection[];
}

export const termsContent: LegalDocument = {
  title: '이용약관',
  effectiveDate: '2026년 3월 1일',
  lastModified: '2026년 3월 1일',
  sections: [
    {
      title: '제1조 (목적)',
      content:
        '본 약관은 아빠의 다락방(이하 "서비스")이 제공하는 모바일 커뮤니티 서비스의 이용 조건 및 절차, 회원과 서비스 간의 권리·의무를 규정함을 목적으로 합니다.',
    },
    {
      title: '제2조 (정의)',
      content:
        '① "서비스"란 아빠의 다락방이 제공하는 모바일 애플리케이션 및 관련 서비스를 말합니다.\n② "회원"이란 본 약관에 동의하고 서비스에 가입한 이용자를 말합니다.\n③ "게시물"이란 회원이 서비스에 작성한 글, 댓글, 이미지 등 일체의 콘텐츠를 말합니다.\n④ "닉네임"이란 회원 식별을 위해 회원이 설정한 이름을 말합니다.',
    },
    {
      title: '제3조 (약관의 효력 및 변경)',
      content:
        '① 본 약관은 서비스 내 공지 또는 개별 통지를 통해 효력이 발생합니다.\n② 서비스는 관련 법령에 위배되지 않는 범위에서 약관을 변경할 수 있으며, 변경 시 7일 전 앱 내 공지합니다.\n③ 회원이 변경된 약관에 동의하지 않는 경우 서비스 이용을 중단하고 탈퇴할 수 있습니다.',
    },
    {
      title: '제4조 (서비스의 제공)',
      content:
        '① 서비스는 다음의 기능을 제공합니다:\n   1. 커뮤니티 게시판 (글 작성, 댓글, 좋아요 등)\n   2. 1:1 채팅 및 그룹 채팅\n   3. 프로필 및 활동 관리\n   4. 알림 서비스\n   5. 검색 서비스\n② 서비스는 운영상 필요한 경우 사전 공지 후 서비스 내용을 변경할 수 있습니다.\n③ 서비스는 정기 점검 등의 사유로 일시 중단될 수 있습니다.',
    },
    {
      title: '제5조 (회원가입)',
      content:
        '① 회원가입은 소셜 로그인(카카오, 네이버, 구글)을 통해 이루어집니다.\n② 가입 시 자녀 정보(나이대), 관심사 등을 입력하며, 이는 맞춤 서비스 제공에 활용됩니다.\n③ 허위 정보를 입력한 경우 서비스 이용이 제한될 수 있습니다.',
    },
    {
      title: '제6조 (불쾌한 콘텐츠 및 악용 행위에 대한 무관용 정책)',
      content:
        '① 서비스는 불쾌한 콘텐츠 및 악용 행위에 대해 무관용 정책(Zero Tolerance Policy)을 시행합니다.\n② 다음에 해당하는 콘텐츠는 절대 허용되지 않으며, 게시 즉시 삭제되고 해당 회원의 계정은 영구 정지됩니다:\n   1. 음란물, 성적으로 노골적인 콘텐츠\n   2. 폭력적이거나 위협적인 콘텐츠\n   3. 혐오 발언, 차별적 표현\n   4. 괴롭힘, 따돌림, 스토킹\n   5. 아동 착취 또는 아동에게 유해한 콘텐츠\n   6. 자해 또는 자살을 조장하는 콘텐츠\n   7. 불법 행위를 조장하거나 촉진하는 콘텐츠\n③ 신고 접수 시 24시간 이내에 해당 콘텐츠를 삭제하고, 위반 회원의 계정을 정지합니다.\n④ 회원은 불쾌한 콘텐츠를 발견 시 즉시 신고 기능을 이용하여 신고할 수 있습니다.\n⑤ 반복적으로 본 정책을 위반하는 회원은 영구적으로 서비스 이용이 제한됩니다.',
    },
    {
      title: '제7조 (회원탈퇴 및 자격상실)',
      content:
        '① 회원은 언제든 서비스 내에서 탈퇴를 요청할 수 있습니다.\n② 탈퇴 시 작성한 게시물은 삭제되지 않으며, 닉네임은 "탈퇴한 회원"으로 표시됩니다.\n③ 다음의 경우 서비스는 사전 통보 없이 회원 자격을 즉시 정지 또는 영구 제한할 수 있습니다:\n   1. 타인의 명예를 훼손하거나 불이익을 주는 행위\n   2. 서비스 운영을 방해하는 행위\n   3. 음란물, 폭력적 콘텐츠를 게시하는 행위\n   4. 상업적 광고를 무단으로 게시하는 행위\n   5. 타인의 개인정보를 무단으로 수집·유포하는 행위\n   6. 제6조의 무관용 정책을 위반하는 행위',
    },
    {
      title: '제8조 (회원의 의무)',
      content:
        '① 회원은 관련 법령, 본 약관, 서비스 이용 안내를 준수해야 합니다.\n② 회원은 타인의 권리를 존중하고 건전한 커뮤니티 문화를 유지해야 합니다.\n③ 회원은 자신의 계정 정보를 안전하게 관리할 책임이 있습니다.\n④ 회원은 불쾌하거나 부적절한 콘텐츠를 발견할 경우 신고 기능을 통해 적극적으로 신고해야 합니다.',
    },
    {
      title: '제9조 (게시물의 관리)',
      content:
        '① 서비스는 다음에 해당하는 게시물을 사전 통보 없이 삭제하거나 비공개 처리할 수 있습니다:\n   1. 법령에 위반되는 내용\n   2. 음란, 폭력적 내용\n   3. 타인의 명예를 훼손하는 내용\n   4. 허위 사실을 유포하는 내용\n   5. 상업적 광고·스팸\n   6. 제6조의 무관용 정책에 위반되는 콘텐츠\n② 게시물에 대한 신고 접수 시 검토 후 24시간 이내에 조치합니다.\n③ 회원은 자신이 작성한 게시물을 언제든 즉시 삭제할 수 있습니다.',
    },
    {
      title: '제10조 (저작권)',
      content:
        '① 회원이 작성한 게시물의 저작권은 해당 회원에게 있습니다.\n② 서비스는 회원의 게시물을 서비스 운영 목적(홍보, 서비스 개선 등)으로 사용할 수 있으며, 이 경우 게시물의 내용을 변경하지 않습니다.\n③ 타인의 저작물을 무단으로 게시한 경우 해당 회원이 모든 법적 책임을 부담합니다.',
    },
    {
      title: '제11조 (면책조항)',
      content:
        '① 서비스는 회원 간 분쟁에 대해 개입하지 않으며 이에 대한 책임을 지지 않습니다.\n② 서비스는 천재지변, 시스템 장애 등 불가항력으로 인한 서비스 중단에 대해 책임을 지지 않습니다.\n③ 회원이 게시한 정보의 정확성·신뢰성에 대해 서비스는 보증하지 않습니다.',
    },
    {
      title: '제12조 (분쟁해결)',
      content:
        '① 서비스와 회원 간 분쟁은 대한민국 법률을 적용합니다.\n② 분쟁 발생 시 상호 협의를 통해 해결하며, 합의가 이루어지지 않을 경우 서울중앙지방법원을 관할 법원으로 합니다.',
    },
    {
      title: '제13조 (부적절한 활동 신고 및 연락처)',
      content:
        '① 부적절한 콘텐츠 또는 활동을 발견한 경우 다음의 방법으로 신고할 수 있습니다:\n   1. 앱 내 신고 기능 이용\n   2. 앱 내 문의하기 기능 이용\n   3. 이메일: sbro@sbrother.co.kr\n② 신고 접수 후 24시간 이내에 검토 및 조치합니다.\n③ 긴급한 사안의 경우 이메일로 직접 연락해주시기 바랍니다.',
    },
  ],
};

export const privacyContent: LegalDocument = {
  title: '개인정보처리방침',
  effectiveDate: '2026년 3월 1일',
  lastModified: '2026년 3월 1일',
  sections: [
    {
      title: '제1조 (개인정보의 수집 항목 및 방법)',
      content:
        '① 서비스는 회원가입 및 서비스 제공을 위해 다음의 개인정보를 수집합니다:\n\n[필수 수집 항목]\n- 소셜 로그인 정보: 이름, 이메일, 프로필 사진 (카카오/네이버/구글 제공)\n- 서비스 이용 기록: 접속 일시, IP 주소, 기기 정보\n\n[선택 수집 항목]\n- 닉네임, 프로필 사진 (직접 설정)\n- 자녀 정보: 나이대, 성별, 자녀 수\n- 관심사 카테고리\n- 거주 지역 (v2 예정)\n\n② 수집 방법: 소셜 로그인 API, 앱 내 직접 입력, 자동 수집 (서비스 이용 기록)',
    },
    {
      title: '제2조 (개인정보의 수집 및 이용 목적)',
      content:
        '수집한 개인정보는 다음의 목적으로 이용합니다:\n① 회원 관리: 회원 식별, 가입·탈퇴 처리, 부정 이용 방지\n② 서비스 제공: 맞춤 콘텐츠 추천, 커뮤니티 기능 제공, 알림 발송\n③ 서비스 개선: 이용 통계 분석, 신규 서비스 개발\n④ 안전한 환경: 부정 이용 탐지, 신고 처리, 분쟁 해결',
    },
    {
      title: '제3조 (개인정보의 보유 및 이용 기간)',
      content:
        '① 회원 탈퇴 시 즉시 파기합니다. 단, 다음의 경우 명시된 기간 동안 보관합니다:\n   - 부정 이용 방지 목적: 탈퇴 후 30일\n   - 전자상거래법에 따른 계약·결제 기록: 5년\n   - 통신비밀보호법에 따른 통신사실 확인 자료: 3개월\n② 보관 기간 경과 후 지체 없이 파기합니다.',
    },
    {
      title: '제4조 (개인정보의 제3자 제공)',
      content:
        '① 서비스는 원칙적으로 회원의 개인정보를 제3자에게 제공하지 않습니다.\n② 다음의 경우 예외로 합니다:\n   1. 회원이 사전에 동의한 경우\n   2. 법령에 의거하여 수사기관의 요청이 있는 경우',
    },
    {
      title: '제5조 (개인정보의 파기)',
      content:
        '① 보유 기간이 경과하거나 처리 목적이 달성된 경우 지체 없이 파기합니다.\n② 파기 방법:\n   - 전자적 파일: 복구 불가능한 방법으로 영구 삭제\n   - 문서: 분쇄 또는 소각',
    },
    {
      title: '제6조 (이용자의 권리와 행사 방법)',
      content:
        '① 회원은 언제든 자신의 개인정보를 열람, 수정, 삭제할 수 있습니다.\n② 개인정보 수정: 앱 내 프로필 수정 기능 이용\n③ 회원 탈퇴: 앱 내 탈퇴 기능 또는 고객센터 문의\n④ 만 14세 미만 아동의 개인정보는 수집하지 않습니다.',
    },
    {
      title: '제7조 (개인정보의 안전성 확보 조치)',
      content:
        '서비스는 개인정보의 안전성 확보를 위해 다음의 조치를 취합니다:\n① 기술적 조치: 데이터 암호화(Firebase 보안), 접근 권한 관리, 보안 프로그램 운영\n② 관리적 조치: 개인정보 취급자 최소화, 정기 교육\n③ 물리적 조치: 서버 접근 통제 (Google Cloud 인프라)',
    },
    {
      title: '제8조 (쿠키 및 자동 수집 장치)',
      content:
        '① 서비스는 앱 사용 편의를 위해 기기 식별자, 앱 사용 데이터를 자동 수집할 수 있습니다.\n② 회원은 기기 설정을 통해 자동 수집을 거부할 수 있으나, 일부 서비스 이용이 제한될 수 있습니다.',
    },
    {
      title: '제9조 (개인정보 보호 책임자)',
      content:
        '개인정보 보호에 관한 문의는 아래 연락처로 문의해주세요:\n- 이메일: sbro@sbrother.co.kr\n- 고객센터: 앱 내 문의하기',
    },
    {
      title: '제10조 (개인정보처리방침의 변경)',
      content:
        '① 본 방침은 관련 법령 및 서비스 정책 변경에 따라 수정될 수 있습니다.\n② 변경 시 7일 전 앱 내 공지하며, 중요 변경은 30일 전 공지합니다.\n\n시행일: 2026년 3월 1일',
    },
  ],
};
